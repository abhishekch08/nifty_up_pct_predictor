...
def test_strategy_report_ranks_only_capped_loss_structures(tmp_path):
    ...
    with Session() as db:
        db.add(Prediction(
            ...
            expected_return=0.006, expected_upper_range=24600, expected_lower_range=24200,
...
def test_strategy_report_can_select_no_trade_for_tiny_edge(tmp_path):
    ...
    with Session() as db:
        db.add(Prediction(
            date=date(2026, 7, 6), next_trading_day=date(2026, 7, 7), model_version="vtest",
            nifty_close=24400, india_vix=12, probability_up=0.501, probability_down=0.499,
            expected_return=0.0001, expected_upper_range=24600, expected_lower_range=24200,
            signal="Neutral / No Edge", regime="Sideways Low Volatility", confidence="Low",
            data_quality="Complete", completeness=0.95, bullish_factors=[], bearish_factors=[]))
        for strike in range(23800, 25001, 100):
            distance = abs(strike - 24400)
            time_value = max(8, 90 - distance * 0.12)
