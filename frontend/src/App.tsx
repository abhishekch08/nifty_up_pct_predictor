...
function Flows() {
  ...
  return <><section className="two-col"><div className="panel wide"><PanelTitle label="Institutional cash flow" note="₹ crore · FII vs DII net activity" help="Daily net buying or selling in the cash market. Positive FII/DII net flow indicates net buying support; negative flow indicates net selling pressure. It is not used for the same session because the official data is known after market close."/>
    {chart.length ? <ChartScroller minWidth={620}><ResponsiveContainer width="100%" height={340}><BarChart data={chart}><CartesianGrid stroke="#1b302c" vertical={false}/><XAxis dataKey="date" hide/><YAxis stroke="#66807a"/><Tooltip contentStyle={tip}/><Bar dataKey="fii_net" fill="#f06a69" radius={[2,2,0,0]}/><Bar dataKey="dii_net" fill="#51e6a6" radius={[2,2,0,0]}/></BarChart></ResponsiveContainer></ChartScroller> : <Empty text="Upload FII/DII CSV from Admin to activate flow analytics."/>}
...
function RecentReturnCalibrationChart({data,height}:{data:RecentCalibrationPoint[];height:number}) {
  ...
  return <ChartScroller minWidth={520}><ResponsiveContainer width="100%" height={height}><LineChart data={data}>
...
function StrategyPriceChart({data}:{data:any[]}) {
  return <ChartScroller minWidth={680}><ResponsiveContainer width="100%" height={350}><LineChart data={data}><CartesianGrid stroke="#d9e1df22" vertical={false}/><XAxis dataKey="spot" tickFormatter={num} stroke="#66807a"/><YAxis tickFormatter={moneyShort} stroke="#66807a"/><Tooltip contentStyle={tip} formatter={(v:any)=>money(Number(v))}/><ReferenceLine y={0} stroke="#829992"/><Line dataKey="target_profit" name="Target-day profit" stroke="#4dd7a2" strokeDasharray="5 4" strokeWidth={2} dot={false}/><Line dataKey="target_loss" name="Target-day loss" stroke="#ff9d9d" strokeDasharray="5 4" strokeWidth={2} dot={false}/><Line dataKey="expiry_profit" name="Expiry profit" stroke="#16a36f" strokeWidth={2.2} dot={false}/><Line dataKey="expiry_loss" name="Expiry loss" stroke="#f06a69" strokeWidth={2.2} dot={false}/></LineChart></ResponsiveContainer></ChartScroller>
}
...
function ChartScroller({children,minWidth=640}:{children:ReactNode;minWidth?:number}){return <div className="chart-scroll"><div className="chart-inner" style={{minWidth}}>{children}</div></div>}
function InfoTip({text,compact=false}:{text:string;compact?:boolean}){return <span className={compact?'info-tip compact':'info-tip'} tabIndex={0} role="button" aria-label={text} onClick={e=>{e.stopPropagation();(e.currentTarget as HTMLElement).focus()}} onKeyDown={e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();(e.currentTarget as HTMLElement).focus()}}}><Info size={compact?12:14}/><span className="info-bubble">{text}</span></span>}
